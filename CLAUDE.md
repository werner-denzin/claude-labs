...
- **The report footer carries a run note: measured time, forecast cost.**
  Generated by `build_card.py --run-note items.json`. The elapsed time is real;
  the cost is a forecast from one calibrated measurement, because a run cannot
  read its own token usage — the line says "estimate, not measured usage" and
  must keep saying it. Re-anchor the coefficients from a fresh measurement rather
  than letting the number quietly rot; the method is in
