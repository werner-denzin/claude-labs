...
flowchart TD
    ...
    TRIAGE{"Claude triage<br>consolidate duplicates<br>score temperature<br>select 20: 10 engineering · 5 strategy<br>3 research · 2 regulation<br>write label + description<br>record blocks as anomalies<br>nominate up to 3 to try"}
...
| `assets/report-template.md` | Shape of the markdown newsletter archived in `reports/`, including `Worth Trying` — what the team could evaluate — and the `Blocked / Unexpected Behaviors` section that the catalog decisions get made from. |
...
| `scripts/build_card.py` | Renderer. Turns `digest.json` into a Teams Adaptive Card, validates the digest, and leaves the coolest cards off the card if the payload would exceed what Teams accepts — never touching the digest, so the report stays complete. `--lens-mix` prints the report header's got/target line. |
...
- **`Worth Trying` recommends, it does not summarise.** At most three cards an
  ...
  bounded time — otherwise it is news, not an experiment.
- **The lens mix is on every report, as got/target.** `engineering 9/10 ·
  strategy 6/5 · research 3/3 · regulation 2/2`, generated by
  `build_card.py --lens-mix` rather than counted. It is what makes a thin
  engineering day visible to the reader and to whoever reads the archive later —
  a target with no feedback loop is a target nobody notices missing.
